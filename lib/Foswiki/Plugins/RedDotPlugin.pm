...
#writeDebug("called renderRedDot($theWeb, $theTopic), parms=".$params->stringify);
    ...
    unless ($theRedirect) {
        my $redirectPref =
          Foswiki::Func::getPreferencesValue("REDDOT_REDIRECT");
        if ($redirectPref) {
            $redirectPref = Foswiki::Func::expandCommonVariables($redirectPref);
            my ( $redirectWeb, $redirectTopic ) =
              Foswiki::Func::normalizeWebTopicName( $baseWeb, $redirectPref );
            $theRedirect =
              Foswiki::Func::getScriptUrl( $redirectWeb, $redirectTopic,
                'view' );
        }
        else {
    ...
    $result .=
      ...
      . ( $counter++ ) . '\' '
      . 'href=\''
      . Foswiki::Func::getScriptUrl( $thisWeb, $thisTopic, 'edit',
        't' => time() );
    $result .= "&redirectto=" . urlEncode($theRedirect)
      if $theRedirect ne "$thisWeb.$thisTopic";
    $result .= '\' ';
    if ($theTitle) {
...
HERE
    ...
    if ( $theAnimate eq 'on' ) {
        Foswiki::Func::addToZone( 'script', 'REDDOTPLUGIN::JS',
